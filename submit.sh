...
usage() {
    ...
    printf "\n\t%-9s  %-40s\n" "0.14"    "[run on signal MC of psi(4260)->DDPI(PHSP) reconstructed @703p01]"
    printf "\n\t%-9s  %-40s\n" "0.14.1"  "Simulation & Reconstruction -- generate signal MC sample"
    printf "\n\t%-9s  %-40s\n" "0.14.2"  "Single D tag -- run on signal MC sample"

    printf "\n\t%-9s  %-40s\n" "0.15"    "[run on signal MC of psi(4260)->DDPIPI(PHSP) (inclusive) reconstructed @703p01]"
    printf "\n\t%-9s  %-40s\n" "0.15.1"  "Simulation & Reconstruction -- generate signal MC sample"
    printf "\n\t%-9s  %-40s\n" "0.15.2"  "Single D tag -- run on signal MC sample"
...
case $option in
    ...
    0.13.1) echo "Simulation & Reconstruction -- generate signal MC sample..."
            cd scripts/gen_script/gen_mc/DDPIPI
            echo "which type of generator do you want to use?"
            read opt
            if [ $opt == "ConExc" ]; then
    ...
    # --------------------------------------------------------------
    #  0.14 signal MC of psi(4260)->DDPI(PHSP) reconstructed @703p01
    # --------------------------------------------------------------
    
    0.14) echo "signal MC of psi(4260)->DDPI(PHSP) reconstructed @705..."
          echo "--> Process: psi(4260)->DDPI"
          echo "--> Generation mode: psi(4260)->DDPI PHSH; D decay D_DALITZ(assignated mode) or PHSP(inlusive mode)"
          ;;

    0.14.1) echo "Simulation & Reconstruction -- generate signal MC sample..."
    ...
    0.14.2) echo "Single D tag -- run on signal MC sample..."
            cd scripts/gen_script/gen_mc/DDPI
            ./subAna_D_D_PI_703p01.sh
            ;;

    # ----------------------------------------------------------------------------
    #  0.15 signal MC of psi(4260)->DDPIPI(PHSP) (inclusive) reconstructed @703p01
    # ----------------------------------------------------------------------------
    
    0.15) echo "signal MC of psi(4260)->DDPIPI(PHSP) reconstructed @705..."
          echo "--> Process: psi(4260)->DDPIPI"
          echo "--> Generation mode: psi(4260)->DDPIPI PHSH"
          ;;

    0.15.1) echo "Simulation & Reconstruction -- generate signal MC sample..."
    ...
    0.15.2) echo "Single D tag -- run on signal MC sample..."
            cd scripts/gen_script/gen_mc/DDPIPIinc
