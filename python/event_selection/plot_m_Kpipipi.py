...
def plot(data_path, pt_title, ecms, xmin, xmax, xbins, mode):
    ...
    set_pavetext(pt)
    pt.Draw()
    if mode == 'raw': pt.AddText('M(K^{-}#pi^{+}#pi^(+)) in signal region')
    if mode == 'side_low': pt.AddText('M(K^{-}#pi^{+}#pi^(+)) in left sideband region')
    if mode == 'side_up': pt.AddText('M(K^{-}#pi^{+}#pi^(+)) in right sideband region')
    pt.AddText(pt_title)
