...
def set_legend(legend, gr1, gr2, gr3, gr4, gr5, gr6, gr7):
    ...
    legend.AddEntry(gr7, '6th  #rightarrow 7th', 'lp')
...
def cal_diff(patch1, patch2, mode):
    ...
    for line1, line2 in zip(XS1, XS2):
        ...
        if xs1 == 0 or xs2 == 0:
            continue
        if fabs((xs2 - xs1)/xs2) > 3 and not mode == 'DDPIPI':
            continue
        if fabs((xs2 - xs1)/xs2) > 5 and mode == 'DDPIPI':
            continue
...
def main(mode):
    ...
    grerr_4_to_5 = cal_diff('round4', 'round5', mode)
    grerr_5_to_6 = cal_diff('round5', 'round6', mode)
    grerr_6_to_7 = cal_diff('round6', 'round7', mode)
    xtitle = 'E_{cms}(GeV)'
    ...
    set_graph_style(grerr_4_to_5, xtitle, ytitle)
    set_graph_style(grerr_5_to_6, xtitle, ytitle)
    set_graph_style(grerr_6_to_7, xtitle, ytitle)
    ...
    grerr_4_to_5.SetMarkerColor(5)
    grerr_4_to_5.SetLineColor(5)
    grerr_4_to_5.SetMarkerStyle(20)
    grerr_5_to_6.SetMarkerColor(6)
    grerr_5_to_6.SetLineColor(6)
    grerr_5_to_6.SetMarkerStyle(20)
    grerr_6_to_7.SetMarkerColor(7)
    grerr_6_to_7.SetLineColor(7)
    grerr_6_to_7.SetMarkerStyle(20)
    if mode == 'total':
        grerr_0_to_1.GetYaxis().SetRangeUser(-1.6, 0.4)
        grerr_0_to_1.Draw('ALP')
        grerr_1_to_2.Draw('LP')
        grerr_2_to_3.Draw('LP')
        grerr_3_to_4.Draw('LP')
        grerr_4_to_5.Draw('LP')
        grerr_5_to_6.Draw('LP')
        grerr_6_to_7.Draw('LP')
    if mode == 'D1_2420':
        grerr_3_to_4.GetYaxis().SetRangeUser(-1.4, 0.8)
        grerr_3_to_4.Draw('ALP')
        grerr_0_to_1.Draw('LP')
        grerr_1_to_2.Draw('LP')
        grerr_2_to_3.Draw('LP')
        grerr_4_to_5.Draw('LP')
        grerr_5_to_6.Draw('LP')
        grerr_6_to_7.Draw('LP')
    if mode == 'psipp':
        grerr_0_to_1.GetYaxis().SetRangeUser(-1.6, 1)
        grerr_0_to_1.Draw('ALP')
        grerr_1_to_2.Draw('LP')
        grerr_2_to_3.Draw('LP')
        grerr_3_to_4.Draw('LP')
        ...
        grerr_6_to_7.Draw('LP')
    if mode == 'DDPIPI':
        grerr_0_to_1.GetYaxis().SetRangeUser(-4.1, 1.2)
        grerr_0_to_1.Draw('ALP')
        grerr_1_to_2.Draw('LP')
        grerr_2_to_3.Draw('LP')
        grerr_3_to_4.Draw('LP')
    ...
    set_legend(legend, grerr_0_to_1, grerr_1_to_2, grerr_2_to_3, grerr_3_to_4, grerr_4_to_5, grerr_5_to_6, grerr_6_to_7)
    legend.Draw()
