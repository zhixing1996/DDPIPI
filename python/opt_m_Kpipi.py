...
def cal_significance(t1, t2, t3, t4, entries1, entries2, entries3, entries4, N, step, ratio1, ratio2, ratio3, ratio4):
    ymax = 0
    NEntry = 0
    B1_list = []
    B2_list = []
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries1*ratio1)):
            t1.GetEntry(j)
            if fabs(t1.m_rawm_D - 1.86965) < (step + i*step):
                B1 = B1 + 1
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries2*ratio2)):
            t2.GetEntry(j)
            if fabs(t2.m_rawm_D - 1.86965) < (step + i*step):
                B2 = B2 + 1
        print 'processing: ' + str(i)
        B2_list.append(B2)
    for i in xrange(N):
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries3*ratio3)):
            t3.GetEntry(j)
            if fabs(t3.m_rawm_D - 1.86965) < (step + i*step):
                S1 = S1 + 1
        print 'processing: ' + str(i)
        S1_list.append(S1)
    print 'Start of incMC2...'
    for i in xrange(N):
        ...
        for j in xrange(int(entries4*ratio4)):
            t4.GetEntry(j)
            if fabs(t4.m_rawm_D - 1.86965) < (step + i*step):
                S2 = S2 + 1
        print 'processing: ' + str(i)
        S2_list.append(S2)
    for i in xrange(N):
...
def plot(incMC1_path, incMC2_path, sigMC1_path, sigMC2_path, pt_title, ecms, ratio1, ratio2, ratio3, ratio4, arrow_left, arrow_bottom, arrow_right, arrow_top):
    try:
    ...
    h_FOM, ientry, arrow_top = cal_significance(t_incMC1, t_incMC2, t_sigMC1, t_sigMC2, entries_incMC1, entries_incMC2, entries_sigMC1, entries_sigMC2, xbins, step, ratio1, ratio2, ratio3, ratio4)
    h_FOM.Draw()
    ...
    range = 'Mass window of K^{-}#pi^{+}#pi^{+}: : [' + mass_low + ', ' + mass_up + '] GeV/c2' + ' with mass window width:     ' + window_width + ' GeV/c2'
    print range
...
def main():
    ...
    if int(energy) == 4360:
        ...
        arrow_bottom = 0
        arrow_top = 29
        plot(incMC1_path, incMC2_path, sigMC1_path, sigMC2_path, pt_title, ecms, ratio1, ratio2, ratio3, ratio4, arrow_left, arrow_bottom, arrow_right, arrow_top)
