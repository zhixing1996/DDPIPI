...
def cal_significance(t1, t2, t3, entries1, entries2, entries3, N, step, ratio1, ratio2, chi2_cut):
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries1*ratio1)):
            t1.GetEntry(j)
            if t1.m_m_Dpi0 > (2.0 + step + i*step) and t1.m_m_pipi > 0.28 and t1.m_rm_Dpipi > 1.855 and t1.m_rm_Dpipi < 1.885 and t1.m_chi2_kf < chi2_cut:
                S1 = S1 + 1
        S1_list.append(S1)
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries2*ratio2)):
            t2.GetEntry(j)
            if t2.m_m_Dpi0 > (2.0 + step + i*step) and t2.m_m_pipi > 0.28 and t2.m_rm_Dpipi > 1.855 and t2.m_rm_Dpipi < 1.885 and t2.m_chi2_kf < chi2_cut:
                S2 = S2 + 1
        S2_list.append(S2)
    for i in xrange(N):
    ...
    for i in xrange(N):
        ...
        for j in xrange(entries3/5):
            t3.GetEntry(j)
            if t3.m_m_Dpi0 > (2.0 + step + i*step) and t3.m_m_pipi > 0.28 and t3.m_rm_Dpipi > 1.855 and t3.m_rm_Dpipi < 1.885 and t3.m_chi2_kf < chi2_cut:
                B = B + 1
        B_list.append(B)
    Ratio_list = []
    for i in xrange(N):
