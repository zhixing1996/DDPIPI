#!/usr/bin/env python
"""
Optiomize mass window of recoiling mass of RM(DPIPI)
"""

__author__ = "Maoqiang JING <jingmq@ihep.ac.cn>"
__copyright__ = "Copyright (c) Maoqiang JING"
__created__ = "[2019-09-12 Tue 11:08]"
...
def cal_significance(t1, t2, t3, entries1, entries2, entries3, M_D, N, step, ratio1, ratio2):
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries1*ratio1)):
            t1.GetEntry(j)
            if abs(t1.m_rm_Dpipi - M_D) < (step + i*step) and t1.m_rawm_D > 1.859332 and t1.m_rawm_D < 1.879968:
                S1 = S1 + 1
        S1_list.append(S1)
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(entries2*ratio2)):
            t2.GetEntry(j)
            if abs(t2.m_rm_Dpipi - M_D) < (step + i*step) and t2.m_rawm_D > 1.859332 and t2.m_rawm_D < 1.879968:
                S2 = S2 + 1
        S2_list.append(S2)
    for i in xrange(N):
    ...
    for i in xrange(N):
        ...
        for j in xrange(entries3/5):
            t3.GetEntry(j)
            if abs(t3.m_rm_Dpipi - M_D) < (step + i*step) and t3.m_rawm_D > 1.859332 and t3.m_rawm_D < 1.879968:
                B = B + 1
        B_list.append(B)
    Ratio_list = []
    for i in xrange(N):
