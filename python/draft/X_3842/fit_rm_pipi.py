...
def fit(path, shape_path, ecms, mode, patch):
    ...
    if mode == 'data' or mode == 'total':
        ...
        if ecms == 4914 or ecms == 4946 or ecms == 4620: pt = TPaveText(0.17, 0.15, 0.3, 0.3, "BRNDC")
        elif ecms < 4600: pt = TPaveText(0.17, 0.15, 0.3, 0.2, "BRNDC")
        else: pt = TPaveText(0.17, 0.15, 0.3, 0.2, "BRNDC")
        set_pavetext(pt)
        pt.Draw()
        if ecms != 'all':
            if ecms == 4420:
