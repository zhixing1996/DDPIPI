...
def fit(ecms, patch, path):
    ...
    xtitle_rm_D = 'RM(D^{+}) (GeV/c^{2})'
    content_rm_D = (xmax_rm_D - xmin_rm_D)/xbins_rm_D * 1000
    ytitle_rm_D = 'Entries/%.1f MeV/c^{2}'%int(content_rm_D )
    format_data_hist(frame_rm_D)
    name_axis(frame_rm_D, xtitle_rm_D, ytitle_rm_D)
    combData.plotOn(frame_rm_D, RooFit.Cut('sample==sample::rm_D'), RooFit.Name('data'))
    sim_pdf.plotOn(frame_rm_D, RooFit.Slice(sample, 'rm_D'), RooFit.Components('pdf_sideband_rm_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kGreen), RooFit.FillStyle(3001), RooFit.FillColor(3), RooFit.LineColor(3), RooFit.VLines(), RooFit.DrawOption('F'))
    if ecms > 4316:
        sim_pdf.plotOn(frame_rm_D, RooFit.Slice(sample, 'rm_D'), RooFit.Components('covpdf_D1_2420_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kRed), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_D, RooFit.Slice(sample, 'rm_D'), RooFit.Components('pdf_psipp_rm_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlue), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_D, RooFit.Slice(sample, 'rm_D'), RooFit.Components('pdf_DDPIPI_rm_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(37), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_D, RooFit.Slice(sample, 'rm_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlack), RooFit.LineWidth(5), RooFit.Name('TotFit'))
    combData.plotOn(frame_rm_D, RooFit.Cut('sample==sample::rm_D'))

    name = []
    name.append('S sample')
    name.append('B sample')
    if ecms > 4316:
        name.append('D_{1}(2420)^{-}D^{+}')
    name.append('#pi^{+}#pi^{-}#psi(3770)')
    name.append('#pi^{+}#pi^{-}D^{+}D^{-}(PHSP)')
    name.append('Total Fit')

    lg_rm_D = TLegend(.18, .5, .38, .88)
    for m in xrange(len(name)):
        ...
        lg_rm_D.SetTextSize(0.07)
    lg_rm_D.SetBorderSize(1)
    lg_rm_D.SetLineColor(0)
    lg_rm_D.SetFillColor(0)
    if ecms == 4230: lg_rm_D.SetHeader('(a)')
    if ecms == 4420: lg_rm_D.SetHeader('(b)')
    if ecms == 4680: lg_rm_D.SetHeader('(c)')
    frame_rm_D.Draw()
    lg_rm_D.Draw()
    ...
    pt_rm_D = TPaveText(0.18, 0.47, 0.38, 0.54, "BRNDC")
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    canvas_name = './figs/simul_fit_' + str(ecms) + '_rm_D.pdf'
    ...
    xtitle_rm_Dmiss = 'RM(D^{-}_{miss}) (GeV/c^{2})'
    content_rm_Dmiss = (xmax_rm_Dmiss - xmin_rm_Dmiss)/xbins_rm_Dmiss * 1000
    ytitle_rm_Dmiss = 'Entries/%.1f MeV/c^{2}'%int(content_rm_Dmiss)
    format_data_hist(frame_rm_Dmiss)
    name_axis(frame_rm_Dmiss, xtitle_rm_Dmiss, ytitle_rm_Dmiss)
    combData.plotOn(frame_rm_Dmiss, RooFit.Cut('sample==sample::rm_Dmiss'), RooFit.Name('data'))
    sim_pdf.plotOn(frame_rm_Dmiss, RooFit.Slice(sample, 'rm_Dmiss'), RooFit.Components('pdf_sideband_rm_Dmiss'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kGreen), RooFit.FillStyle(3001), RooFit.FillColor(3), RooFit.LineColor(3), RooFit.VLines(), RooFit.DrawOption('F'))
    if ecms > 4316:
        sim_pdf.plotOn(frame_rm_Dmiss, RooFit.Slice(sample, 'rm_Dmiss'), RooFit.Components('covpdf_D1_2420_D'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kRed), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_Dmiss, RooFit.Slice(sample, 'rm_Dmiss'), RooFit.Components('pdf_psipp_rm_Dmiss'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlue), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_Dmiss, RooFit.Slice(sample, 'rm_Dmiss'), RooFit.Components('pdf_DDPIPI_rm_Dmiss'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(37), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_Dmiss, RooFit.Slice(sample, 'rm_Dmiss'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlack), RooFit.LineWidth(5), RooFit.Name('TotFit'))
    ...
    lg_rm_Dmiss = TLegend(.18, .5, .38, .88)
    for m in xrange(len(name)):
        ...
        lg_rm_Dmiss.SetTextSize(0.07)
    lg_rm_Dmiss.SetBorderSize(1)
    lg_rm_Dmiss.SetLineColor(0)
    lg_rm_Dmiss.SetFillColor(0)
    if ecms == 4230: lg_rm_Dmiss.SetHeader('(d)')
    if ecms == 4420: lg_rm_Dmiss.SetHeader('(e)')
    if ecms == 4680: lg_rm_Dmiss.SetHeader('(f)')
    frame_rm_Dmiss.Draw()
    lg_rm_Dmiss.Draw()
    ...
    for i in xrange(xbins_rm_Dmiss):
        ...
        if ytot >= 7:
            ...
            nbin += 1
            ytot, avg, eyl, eyh = 0, 0, 0, 0
    pt_rm_Dmiss = TPaveText(0.18, 0.47, 0.38, 0.54, "BRNDC")
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    canvas_name = './figs/simul_fit_' + str(ecms) + '_rm_Dmiss.pdf'
    c2.SaveAs(canvas_name)

    c3 = TCanvas('c3', 'c3', 1000, 700)
    c3.cd()
    frame_rm_pipi = rm_pipi.frame(RooFit.Bins(xbins_rm_pipi), RooFit.Title('rm_pipi'))
    xtitle_rm_pipi = 'RM(#pi_{d}^{+}#pi_{d}^{-}) (GeV/c^{2})'
    content_rm_pipi = (xmax_rm_pipi - xmin_rm_pipi)/xbins_rm_pipi * 1000
    ytitle_rm_pipi = 'Entries/%.1f MeV/c^{2}'%int(content_rm_pipi)
    format_data_hist(frame_rm_pipi)
    name_axis(frame_rm_pipi, xtitle_rm_pipi, ytitle_rm_pipi)
    combData.plotOn(frame_rm_pipi, RooFit.Cut('sample==sample::rm_pipi'), RooFit.Name('data'))
    sim_pdf.plotOn(frame_rm_pipi, RooFit.Slice(sample, 'rm_pipi'), RooFit.Components('pdf_sideband_rm_pipi'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kGreen), RooFit.FillStyle(3001), RooFit.FillColor(3), RooFit.LineColor(3), RooFit.VLines(), RooFit.DrawOption('F'))
    if ecms > 4316:
        sim_pdf.plotOn(frame_rm_pipi, RooFit.Slice(sample, 'rm_pipi'), RooFit.Components('pdf_D1_2420_rm_pipi'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kRed), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_pipi, RooFit.Slice(sample, 'rm_pipi'), RooFit.Components('covpdf_psipp'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlue), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_pipi, RooFit.Slice(sample, 'rm_pipi'), RooFit.Components('pdf_DDPIPI_rm_pipi'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(37), RooFit.LineWidth(5), RooFit.LineStyle(kDashed))
    sim_pdf.plotOn(frame_rm_pipi, RooFit.Slice(sample, 'rm_pipi'), RooFit.ProjWData(RooArgSet(sample), combData), RooFit.LineColor(kBlack), RooFit.LineWidth(5), RooFit.Name('TotFit'))
    combData.plotOn(frame_rm_pipi, RooFit.Cut('sample==sample::rm_pipi'))
    if ecms == 4680:
        frame_rm_pipi.GetYaxis().SetRangeUser(0, 350)
    ...
    for m in xrange(len(name)):
        ...
        lg_rm_pipi.SetTextSize(0.07)
    lg_rm_pipi.SetBorderSize(1)
    lg_rm_pipi.SetLineColor(0)
    lg_rm_pipi.SetFillColor(0)
    if ecms == 4230: lg_rm_pipi.SetHeader('(g)')
    if ecms == 4420: lg_rm_pipi.SetHeader('(h)')
    if ecms == 4680: lg_rm_pipi.SetHeader('(i)')
    frame_rm_pipi.Draw()
    lg_rm_pipi.Draw()
    ...
    for i in xrange(xbins_rm_pipi):
        ...
        if ytot >= 7:
            ...
            nbin += 1
            ytot, avg, eyl, eyh = 0, 0, 0, 0
    pt_rm_pipi = TPaveText(0.58, 0.46, 0.78, 0.53, "BRNDC")
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    canvas_name = './figs/simul_fit_' + str(ecms) + '_rm_pipi.pdf'
