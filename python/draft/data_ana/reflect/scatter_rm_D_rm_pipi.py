...
def usage():
    sys.stdout.write('''
NAME
    scatter_rm_D_rm_pipi.py

SYNOPSIS
    ./scatter_rm_D_rm_pipi.py [ecms] [charm]
...
def plot(path, leg_title, ecms, xmin, xmax, ymin, ymax, bins, charm):
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    mbc.SaveAs('./figs/scatter_rm_D_rm_pipi_'+str(ecms)+'_'+str(charm)+'.pdf')
...
if __name__ == '__main__':
    ...
    leg_title = ''
    if charm == 1: leg_title = '(c)'
    if charm == -1: leg_title = '(d)'
    xmin = 2.2
    xmax = 2.6
    ymin = 3.79
    ymax = 4.12
