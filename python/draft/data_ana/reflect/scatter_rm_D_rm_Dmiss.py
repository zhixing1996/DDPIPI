...
def usage():
    sys.stdout.write('''
NAME
    scatter_rm_D_rm_Dmiss.py

SYNOPSIS
    ./scatter_rm_D_rm_Dmiss.py [ecms] [charm]
...
def fill(t, h, charm):
    for ientry in xrange(t.GetEntries()):
        t.GetEntry(ientry)
        if t.m_rm_Dmiss < 3.79 and t.m_charm != charm:
            continue
        h.Fill(t.m_rm_D, t.m_rm_Dmiss)
...
def plot(path, leg_title, ecms, xmin, xmax, ymin, ymax, bins, charm):
    ...
    pt = TPaveText(0.2, 0.7, 0.4, 0.85, "BRNDC")
    set_pavetext(pt)
    pt.Draw()
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    mbc.SaveAs('./figs/scatter_rm_D_rm_Dmiss_'+str(ecms)+'_'+str(charm)+'.pdf')
...
if __name__ == '__main__':
    ...
    if charm == 1: leg_title = '(a)'
    if charm == -1: leg_title = '(b)'
    xmin = 2.2
    xmax = 2.6
    ymin = 2.2
    ymax = 2.6
