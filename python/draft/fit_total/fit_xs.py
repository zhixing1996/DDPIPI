...
import logging
from math import *
from style import width, set_pub_style, set_prelim_style, name_axis, format_data_hist, set_arrow, set_pavetext, position_convert, set_legend, format_mc_hist, format_data_graph
...
def construct_A2():
    ...
    tfunc.SetParameters(param_init)
...
def fit():
    mbc = TCanvas('mbc', 'mbc', 315, 125, 1000, 700)
    mbc.Range(3.941142,-39.05958,5.141918,132.1433)
    mbc.cd()
    ...
    if 'BW_4230_a' not in components and 'PHSP_a' not in components:
        format_data_graph(geexs)
        name_axis(geexs, xtitle, ytitle)
        geexs.GetYaxis().SetRangeUser(-15., 1.3 * xs_max)
    else:
        format_data_graph(geexs)
        name_axis(geexs, xtitle, ytitle)
        geexs.GetYaxis().SetRangeUser(-5., 1.3 * xs_max)
    geexs.Draw('ap')
    tfunc = construct_A2()
    geexs.Fit(tfunc, 'E')
    tfunc.Draw('same')
    ...
    if len(components) == 3 and ('BW_4390' in components and 'BW_4700' in components and 'PHSP' in components): line = '(a)'
    elif len(components) == 2 and ('BW_4390' in components and 'BW_4700' in components): line = '(b)'
    ...
    if 'BW_4230_a' not in components and 'PHSP_a' not in components: legend = TLegend(0.75, 0.75, 0.85, 0.85)
    else: legend = TLegend(0.75, 0.75, 0.85, 0.85)
    if len(components) == 3 and ('BW_4390' in components and 'BW_4700' in components and 'PHSP' in components): line = '(a)'
    elif len(components) == 2 and ('BW_4390' in components and 'BW_4700' in components): line = '(b)'
