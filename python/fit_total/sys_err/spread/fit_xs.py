...
import xs_func_four_body as xs_func_four_body
...
def construct_A2():
    ...
    tffunc = TF1('tffunc', ffunc_DDPIPI, xmin, xmax, len(param_init))
    rand_param = []
    for ilimit, low, high in param_range:
        value = random.uniform(low, high)
        tffunc.SetParameter(ilimit, value)
        tffunc.SetParLimits(ilimit, low, high)
        rand_param.append([ilimit, value])
    tffunc.SetLineColor(2)
    return rand_param, tffunc
...
def fit(iround):
    ...
    set_graph_style(geexs, xtitle, ytitle, xmin = -15., xmax = xs_max * 1.7)
    geexs.Draw('ap')
    rand_param, tffunc = construct_A2()
    result = geexs.Fit(tffunc, 'S', '')
    flag = 0
    if result.IsValid(): flag = 1
    tffunc.Draw('same')
    _, param_range = read_params()
    for i in range(tffunc.GetNumberFreeParameters()):
        fit_value = tffunc.GetParameter(i)
    ...
    for component in components:
        ...
        for i in range(start_param_pos, start_param_pos + len_param): 
            par.append(tffunc.GetParameter(i))
    ...
    if flag != 0:
        ...
        if not os.path.exists('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/txts/'):
            os.makedirs('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/txts/')
        with open('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/txts/fit_result' + label + '_' + str(iround) + '.txt', 'w') as f:
            f.write(str(iround) + ' ' + str(chi2) + ' ')
            for param in rand_param:
                ilimit, par_ini = param
                f.write(str(ilimit) + ' ' + str(par_ini) + ' ')

        with open('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/txts/likelihood' + label + '_' + str(iround) + '.txt', 'w') as f:
            f.write(str(chi2) + '\n' + str(ndf))

        with open('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/txts/params' + label + '_' + str(iround) + '.txt', 'w') as f:
            for key, value in par_index.items():
                out = key + ' ' + str(tffunc.GetParameter(value)) + ' ' + str(tffunc.GetParError(value)) + '\n'
                f.write(out)

        legend = TLegend(0.2, 0.6, 0.5, 0.88)
        set_legend(legend, tffunc, names, funcs, line)
        legend.Draw()

        if not os.path.exists('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/figs/'):
            os.makedirs('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/figs/')

        mbc.SaveAs('/scratchfs/bes/jingmq/bes/DDPIPI/v0.2/run/ana/fit/spread/figs/xs' + label + '_' + str(iround) + '.pdf')
