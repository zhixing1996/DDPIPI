...
def sys_err(patch):
    ...
    path_sys_err = './txts/sys_err_sigma_shape.txt'
    ...
    for line_xs_old in lines_xs_old:
        ...
        for line_xs_new in lines_xs_new:
            ...
            if ecms < 4.316: diff, err_diff = 0., 0.
            else:
                xs_new = fargs_new[1]
    ...
    for line_xs_old in lines_xs_old:
        fargs_old = map(float, line_xs_old.strip().split())
        ecms = fargs_old[0]
        if ecms > 4.316: out = str(ecms) + '\t' + str(round(diff*100, 1)) + '\n'
        else: out = str(ecms) + '\t' + str(round(0.*100, 1)) + '\n'
        f_sys_err.write(out)
