...
def cal_diff():
    ...
    dic = {}
    TYPES = ['background_shape', 'BF', 'fit_range', 'IntLum', 'K_p', 'm_pipi', 'PID', 'tracking', 'VP', 'VrVz', 'width', 'window', 'sigma_shape']
    ...
    for ECM in ECMS:
        SYS = []
        for TYPE in TYPES:
            if TYPE == 'background_shape' or TYPE == 'fit_range' or TYPE == 'sigma_shape': f_type = open('../'+TYPE+'/txts/sys_err_'+TYPE+'.txt', 'r')
            else: f_type = open('../../../sys_err/'+TYPE+'/txts/sys_err_'+TYPE+'.txt', 'r')
