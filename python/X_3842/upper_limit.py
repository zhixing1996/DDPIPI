...
def upper_limit(path, ecms, patch):
    ...
    with open('./txts/xs_info_' + str(ecms) + '_read_' + patch + '.txt') as f:
        for line in f.readlines():
            fargs = map(float, line.strip('\n').strip().split())
            f_K_p, f_m_pipi, f_VrVz, f_m_Kpipi, f_rm_Dpipi, eff_X_3842, ISR_X_3842, VP, lum, Br = fargs[3], fargs[4], fargs[5], fargs[6], fargs[7], fargs[8]/100., fargs[9], fargs[10], fargs[11], fargs[12]/100.
    f = f_K_p * f_m_pipi * f_VrVz * f_m_Kpipi * f_rm_Dpipi
    muldel = 1./(2*f*eff_X_3842*ISR_X_3842*VP*Br*lum)
    FCN_sum = 0
