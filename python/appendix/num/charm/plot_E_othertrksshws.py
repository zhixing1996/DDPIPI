...
def E_othertrksshws_fill(t, h, num_charm):
    for ientry in xrange(t.GetEntries()):
        t.GetEntry(ientry)
        if t.m_n_p == 0 and t.m_n_pbar == 0 and t.m_charm == num_charm:
            h.Fill(t.m_E_othershws + t.m_E_othertrks)
...
def plot(path, ecms, xmin, xmax, num_charm):
    ...
    mbc.SaveAs('./figs/E_othertrksshws_'+str(ecms)+'_'+str(num_charm)+'.pdf')
