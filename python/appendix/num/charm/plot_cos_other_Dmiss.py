#!/usr/bin/env python
"""
Plot cos theta of other tracks and showers and missed D
"""

__author__ = "Maoqiang JING <jingmq@ihep.ac.cn>"
__copyright__ = "Copyright (c) Maoqiang JING"
__created__ = "[2020-10-18 Sun 16:51]"
...
def usage():
    sys.stdout.write('''
NAME
    plot_cos_other_Dmiss.py

SYNOPSIS
    ./plot_cos_other_Dmiss.py [ecms] [num_charm]
...
def cos_other_Dmiss_fill(t, h, num_charm):
    for ientry in xrange(t.GetEntries()):
        t.GetEntry(ientry)
        if t.m_n_p == 0 and t.m_n_pbar == 0 and t.m_charm == num_charm:
            h.Fill(t.m_cos_other_Dmiss)
...
def plot(path, ecms, xmin, xmax, num_charm):
    ...
    mbc.SaveAs('./figs/cos_other_Dmiss_'+str(ecms)+'_'+str(num_charm)+'.pdf')
