#!/usr/bin/env python
"""
Plot cos theta of other tracks plus showers
"""

__author__ = "Maoqiang JING <jingmq@ihep.ac.cn>"
__copyright__ = "Copyright (c) Maoqiang JING"
__created__ = "[2020-10-18 Sun 19:19]"
...
def usage():
    sys.stdout.write('''
NAME
    plot_cos_othertrksshws.py

SYNOPSIS
    ./plot_cos_othertrksshws.py [ecms] [num_charm]
...
def cos_other_trksshws_fill(t, h, num_charm):
    for ientry in xrange(t.GetEntries()):
        t.GetEntry(ientry)
        if t.m_n_p == 0 and t.m_n_pbar == 0 and t.m_charm == num_charm:
            h.Fill(t.m_cos_otherall)
...
def plot(path, ecms, xmin, xmax, num_charm, ymax):
    try:
        f_data = TFile(path[0])
    ...
    xbins = 50
    ytitle = 'Eentries'
    xtitle = 'Cos(OtherTrksShws)'

    h_data = TH1F('data', 'data', xbins, xmin, xmax)
    set_histo_style(h_data, xtitle, ytitle, 1, -1, ymax)
    cos_other_trksshws_fill(t_data, h_data, num_charm)
    
    h_side1 = TH1F('side1', 'side1', xbins, xmin, xmax)
    set_histo_style(h_side1, xtitle, ytitle, 3, 3004, ymax)
    cos_other_trksshws_fill(t_side1, h_side1, num_charm)
    
    h_side2 = TH1F('side2', 'side2', xbins, xmin, xmax)
    set_histo_style(h_side2, xtitle, ytitle, 3, 3004, ymax)
    cos_other_trksshws_fill(t_side2, h_side2, num_charm)
    
    h_side3 = TH1F('side3', 'side3', xbins, xmin, xmax)
    set_histo_style(h_side3, xtitle, ytitle, 3, 3004, ymax)
    cos_other_trksshws_fill(t_side3, h_side3, num_charm)
    
    h_side4 = TH1F('side4', 'side4', xbins, xmin, xmax)
    set_histo_style(h_side4, xtitle, ytitle, 3, 3004, ymax)
    cos_other_trksshws_fill(t_side4, h_side4, num_charm)
    ...
    legend = TLegend(0.55, 0.2, 0.8, 0.45)
    ...
    mbc.SaveAs('./figs/cos_othertrksshws_'+str(ecms)+'_'+str(num_charm)+'.pdf')
...
if __name__ == '__main__':
    ...
    xmax = 1.1
    ymax = 50
    plot(path, ecms, xmin, xmax, num_charm, ymax)
