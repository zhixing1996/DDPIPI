#!/usr/bin/env python
"""
Plot E(othertrks) vs E(othershws)
"""
...
def fill(t, h, num_charm):
    for ientry in xrange(t.GetEntries()):
        ...
        if t.m_n_p == 0 and t.m_n_pbar == 0 and t.m_charm == num_charm:
            h.Fill(t.m_E_othertrks, t.m_E_othershws)
...
def plot(path, leg_title, ecms, xmin, xmax, xbins, ymin, ymax, ybins, sample, num_charm):
    ...
    ytitle = 'E(OtherShws) (GeV)'

    if sample == 'data':
        h_data = TH2F('scatter_data', 'scatter plot of E(othertrks) and E(othershws)', xbins, xmin, xmax, ybins, ymin, ymax)
        set_histo_style(h_data, xtitle, ytitle)
        fill(t_data, h_data, num_charm)
        h_data.Draw('COLZ')
    if sample == 'sideband':
        h_side1 = TH2F('scatter_side1', 'scatter plot of E(othertrks) and E(othershws)', xbins, xmin, xmax, ybins, ymin, ymax)
        set_histo_style(h_side1, xtitle, ytitle)
        fill(t_side1, h_side1, num_charm)
        h_side2 = TH2F('scatter_side2', 'scatter plot of E(othertrks) and E(othershws)', xbins, xmin, xmax, ybins, ymin, ymax)
        set_histo_style(h_side2, xtitle, ytitle)
        fill(t_side2, h_side2, num_charm)
        h_side3 = TH2F('scatter_side3', 'scatter plot of E(othertrks) and E(othershws)', xbins, xmin, xmax, ybins, ymin, ymax)
        set_histo_style(h_side3, xtitle, ytitle)
        fill(t_side3, h_side3, num_charm)
        h_side4 = TH2F('scatter_side4', 'scatter plot of E(othertrks) and E(othershws)', xbins, xmin, xmax, ybins, ymin, ymax)
    ...
    mbc.SaveAs('./figs/scatter_E_othertrks_E_othershws_'+str(ecms)+'_'+sample+'_'+str(num_charm)+'.pdf')
