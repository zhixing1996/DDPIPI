...
def shape_side(ecms, path_in, path_out):
    ...
    with open('./txts/scale_factor_rm_Dpipi_' + str(ecms) + '.txt', 'r') as f:
        for line in f.readlines():
            fargs = map(float, line.strip('\n').strip().split())
    scale_rm_Dpipi = fargs[0]
    with open('./txts/scale_factor_m_Kpipi_' + str(ecms) + '.txt', 'r') as f:
        for line in f.readlines():
            fargs = map(float, line.strip('\n').strip().split())
    scale_m_Kpipi = fargs[0]
    h_side_rm_pipi = TH1F('h_side_rm_pipi', '', xbins_rm_pipi, xmin_rm_pipi, xmax_rm_pipi)
    h_side1_rm_pipi.Scale(scale_rm_Dpipi)
    h_side_rm_pipi.Add(h_side1_rm_pipi)
    h_side2_rm_pipi.Scale(scale_m_Kpipi)
    h_side_rm_pipi.Add(h_side2_rm_pipi)
    h_side3_rm_pipi.Scale(scale_rm_Dpipi/2.)
    h_side_rm_pipi.Add(h_side3_rm_pipi, -1)
    h_side4_rm_pipi.Scale(scale_rm_Dpipi/2.)
    h_side_rm_pipi.Add(h_side4_rm_pipi, -1)
    ...
    h_side1_rm_D.Scale(scale_rm_Dpipi)
    h_side_rm_D.Add(h_side1_rm_D)
    h_side2_rm_D.Scale(scale_m_Kpipi)
    h_side_rm_D.Add(h_side2_rm_D)
    h_side3_rm_D.Scale(scale_rm_Dpipi/2.)
    h_side_rm_D.Add(h_side3_rm_D, -1)
    h_side4_rm_D.Scale(scale_rm_Dpipi/2.)
    h_side_rm_D.Add(h_side4_rm_D, -1)
    ...
    h_side1_rm_Dmiss.Scale(scale_rm_Dpipi)
    h_side_rm_Dmiss.Add(h_side1_rm_Dmiss)
    h_side2_rm_Dmiss.Scale(scale_m_Kpipi)
    h_side_rm_Dmiss.Add(h_side2_rm_Dmiss)
    h_side3_rm_Dmiss.Scale(scale_rm_Dpipi/2.)
    h_side_rm_Dmiss.Add(h_side3_rm_Dmiss, -1)
    h_side4_rm_Dmiss.Scale(scale_rm_Dpipi/2.)
    h_side_rm_Dmiss.Add(h_side4_rm_Dmiss, -1)
...
def main():
    ...
    path_out = '/besfs5/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/scale_factor/shape_side_'+str(ecms)+'.root'
