...
def make_sys():
    sample, sys_err  = [], []
    with open('./txts/sys_err_D1_2420_shape.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            try:
    ...
    if not os.path.exists('./texs/'):
        os.makedirs('./texs/')
    with open('./texs/D1_2420_shape_sys.tex', 'w') as f:
        f.write('\\begin{table}[htp]\n')
        f.write('\t\centering\n')
        f.write('\t\caption{Systematic uncertainty caused by $D_{1}(2420)^{+}$ shape.}\n')
