...
def cal(path):
    ...
    sys_err = sqrt(eff_data_err**2/eff_data**2 + eff_KsKpi_err**2/eff_KsKpi**2)
    ...
    with open('./txts/sys_err_K_p.txt', 'w') as f:
        for ecm in ecms:
...
if __name__ == '__main__':
    ...
    path = []
    path.append('/besfs/groups/cal/dedx/$USER/bes/KsKpi/run/KsKpi/anaroot/data/' + str(ecms) + '/data_' + str(ecms) + '_signal.root')
    path.append('/besfs/groups/cal/dedx/jingmq/bes/KsKpi/run/KsKpi/anaroot/mc/KsKpi/' + str(ecms) + '/mc_KsKpi_' + str(ecms) + '_signal.root')
    cal(path)
