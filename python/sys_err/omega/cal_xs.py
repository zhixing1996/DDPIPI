...
def xs(ecms, patch, signal_path, sideband_path):
    ...
    N_data_sideband, Err_data_sideband = 0, 0
    ...
    with open(omega_path, 'r') as f:
        ...
        for line in lines:
            ...
            count += 1
    if not ecms == 4210:
        omega_D1_2420 = omega[0] + omega_err[0]
        omega_psipp = omega[1] + omega_err[1]
        omega_DDPIPI = omega[2] - omega_err[0] - omega_err[1]
    else:
        omega_D1_2420 = omega[0]
        omega_psipp = omega[1]
        omega_DDPIPI = omega[2]
