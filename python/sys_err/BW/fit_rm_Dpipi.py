...
def fit(path, shape_path, ecms, mode, patch):
    ...
    while True:
        ...
        if is_OK == -1: break
        if ecms == 4190 and chi2_ndf < 6: break
        if (is_OK == 3 and status == 0 and chi2_ndf < 2.7 and ecms < 4221): break
...
def main():
    ...
    if mode == 'psipp' or mode == 'D1_2420':
        path.append('/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/BW/sigMC_' + mode + '_' + str(ecms) + '_raw_after.root')
        shape_path = '/besfs5/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/BW/shape_' + mode + '_' + str(ecms) + '_signal.root'
    if mode == 'DDPIPI':
        path.append('/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/BW/sigMC_D_D_PI_PI_' + str(ecms) + '_raw_after.root')
