...
def fit(path, shape_path, ecms, mode, patch):
    ...
    while True:
        ...
        if mode == 'data' or mode == 'DDPIPI' or mode == 'psipp' or mode == 'D1_2420':
        ...
        if not os.path.exists('./txts/'):
            os.makedirs('./txts/')
        path_sig = './txts/' + mode + '_sideband_events_'+ str(ecms) +'_' + patch + '.txt'
...
def main():
    ...
    if mode == 'data':
        ...
        shape_path = '/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/mixed/sys_err/BW/shape_' + str(ecms) + '_mixed.root'
    if mode == 'psipp' or mode == 'D1_2420':
        path.append('/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/BW/sigMC_' + mode + '_' + str(ecms) + '_raw_sideband_after.root')
        shape_path = '/besfs5/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/BW/shape_' + mode + '_' + str(ecms) + '_signal.root'
    if mode == 'DDPIPI':
        path.append('/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/BW/sigMC_D_D_PI_PI_' + str(ecms) + '_raw_sideband_after.root')
        shape_path = '/besfs5/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/BW/shape_D_D_PI_PI_' + str(ecms) + '_signal.root'
