...
def make_sys():
    sample, sys_err  = [], []
    with open('./txts/sys_err_BW.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            try:
    ...
    if not os.path.exists('./texs/'):
        os.makedirs('./texs/')
    with open('./texs/BW_sys.tex', 'w') as f:
        f.write('\\begin{table}[htp]\n')
        f.write('\t\centering\n')
        f.write('\t\caption{Systematic uncertainty caused by the added Breight-Wigners in $e^{+}e^{-}\\rightarrow D_{1}(2420)^{+}D^{-}\\rightarrow \pi^{+}\pi^{-}D^{+}D^{-}$, $e^{+}e^{-}\\rightarrow \pi^{+}\pi^{-}\psi(3770)\\rightarrow \pi^{+}\pi^{-}D^{+}D^{-}$ and $e^{+}e^{-}\\rightarrow \pi^{+}\pi^{-}D^{+}D^{-}(\\textsc{PHSP})$ cross section line-shapes.}\n')
