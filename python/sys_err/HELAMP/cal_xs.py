...
def xs(ecms, patch, signal_path, sideband_path):
    ...
    N_D1_2420_001000 = fargs[0]

    N_D1_2420_100010 = 0.
    fargs = readN(signal_path[2])
    N_D1_2420_100010 = fargs[0]

    fargs = readN(signal_path[3])
    N_psipp = fargs[0]

    fargs = readN(signal_path[4])
    N_DDPIPI = fargs[0]
    ...
    N_D1_2420_001000_sideband = 0.
    fargs = readN(sideband_path[1])
    N_D1_2420_001000_sideband = fargs[0]

    N_D1_2420_100010_sideband = 0.
    fargs = readN(sideband_path[2])
    N_D1_2420_100010_sideband = fargs[0]

    fargs = readN(sideband_path[3])
    N_psipp_sideband = fargs[0]

    fargs = readN(sideband_path[4])
    ...
    eff_D1_2420_001000 = 0.
    eff_D1_2420_100010 = 0.
    if ecms == 4420 or ecms == 4680:
        eff_D1_2420_001000 = (N_D1_2420_001000 - N_D1_2420_001000_sideband/2.)/100000.
        eff_D1_2420_100010 = (N_D1_2420_100010 - N_D1_2420_100010_sideband/2.)/100000.
    else:
        eff_D1_2420_001000 = (N_D1_2420_001000 - N_D1_2420_001000_sideband/2.)/50000.
        eff_D1_2420_100010 = (N_D1_2420_100010 - N_D1_2420_100010_sideband/2.)/50000.

    fargs = readN('./txts/ratio_' + str(ecms) + '.txt')
    R_001000, R_100010 = fargs[0], fargs[1]
    eff_D1_2420 = R_001000*eff_D1_2420_001000 + R_100010*eff_D1_2420_100010
    ...
    with open(psipp_path, 'r') as f:
        for line in f.readlines():
            ...
            try:
                ...
                '''
                '''
    with open('./txts/factor_info_' + str(ecms) + '_D1_2420_001000_' + patch + '.txt', 'r') as f:
        for line in f.readlines():
            fargs = map(float, line.strip().strip('\n').split())
            ISR_D1_2420_001000, VP = fargs[0], fargs[1]
    with open('./txts/factor_info_' + str(ecms) + '_D1_2420_100010_' + patch + '.txt', 'r') as f:
        for line in f.readlines():
            fargs = map(float, line.strip().strip('\n').split())
            ISR_D1_2420_100010, VP = fargs[0], fargs[1]
    ISR_D1_2420 = R_001000*ISR_D1_2420_001000 + R_100010*ISR_D1_2420_100010
    ...
    if omega_D1_2420 == 0:
        ...
        flag_D1_2420 = 1
        eff_ISR_VP_D1_2420 = (R_001000*eff_D1_2420_001000*ISR_D1_2420_001000 + R_100010*eff_D1_2420_100010*ISR_D1_2420_100010)*omega_D1_2420*VP*factor_K_p*factor_m_pipi*factor_VrVz*factor_m_Kpipi*factor_rm_Dpipi
    if omega_psipp == 0:
        flag_psipp = 0
        eff_ISR_VP_psipp = 1
    else:
...
if __name__ == '__main__':
    ...
    signal_path.append('./txts/D1_2420_001000_events_' + str(ecms) + '_' + patch + '.txt')
    signal_path.append('./txts/D1_2420_100010_events_' + str(ecms) + '_' + patch + '.txt')
    ...
    sideband_path.append('./txts/D1_2420_001000_sideband_events_' + str(ecms) + '_' + patch + '.txt')
    sideband_path.append('./txts/D1_2420_100010_sideband_events_' + str(ecms) + '_' + patch + '.txt')
