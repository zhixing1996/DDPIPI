...
def fit(path, shape_path, ecms, mode, patch):
    ...
    while True:
        ...
        if (is_OK == 3 and status == 0 and chi2_ndf < 1.9): break
...
def main():
    ...
    path = []
    shape_path = ''
    path.append('/besfs5/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sigMC_D1_2420_' + str(ecms) + '_raw_after.root')
