...
def main():
    ...
    if mode == 'MC_mix':
        ...
        if ecms > 4316:
            ...
            path_in.append('/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/psipp_shape/shape_psipp_' + str(ecms) + '_signal.root')
    ...
    if mode == 'raw':
        ...
        path_in = '/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/psipp/'+str(ecms)+'/sys_err/psipp_shape/sigMC_psipp_'+str(ecms)+'_raw.root'
        path_out = '/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/psipp_shape/shape_psipp_'+str(ecms)+'_raw.root'
        shape_MC_raw(ecms, mode, path_in, path_out, xmin, xmax, xbins)
        path_IN = []
        path_OUT = []
        if ecms > 4316: path_IN.append('/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/shape_D1_2420_' + str(ecms) + '_raw.root')
        path_IN.append('/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/psipp_shape/shape_psipp_' + str(ecms) + '_raw.root')
        path_IN.append('/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/shape_D_D_PI_PI_' + str(ecms) + '_raw.root')
        path_OUT = '/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/mixed/sys_err/psipp_shape/shape_' + str(ecms) + '_mixed_raw.root'
        mix(path_IN, path_OUT, ecms, xmin, xmax, xbins, patch)
