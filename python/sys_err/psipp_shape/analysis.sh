...
usage() {
    ...
    printf "\n\t%-9s  %-40s\n" "0.1.3" "Simulation & Reconstruction -- generate psipp MC sample(@707)"
    printf "\n\t%-9s  %-40s\n" "0.1.4" "Single D tag -- run on psipp MC sample(@703p01)"
    printf "\n\t%-9s  %-40s\n" "0.1.5" "Single D tag -- run on psipp MC sample(@705)"
    printf "\n\t%-9s  %-40s\n" "0.1.6" "Single D tag -- run on psipp MC sample(@707)"
    printf "\n\t%-9s  %-40s\n" "0.1.7" "Get samples -- extracte useful info: signal region and sideband region, ISR factors"
    printf "\n\t%-9s  %-40s\n" "0.1.8" "Get samples -- apply cuts: psipp MC"
...
case $option in
    ...
    0.1.3) echo "Simulation & Reconstruction -- generating psipp MC sample(@707)..."
           cd scripts/mc/psipp
           ./subSimRec_psipp_707.sh
           ;;

    0.1.4) echo "Single D tag -- running on psipp MC sample(@703p01)..."
           cd scripts/mc/psipp
           ./subAna_psipp_703p01.sh
           ;;

    0.1.5) echo "Single D tag -- running on psipp MC sample(@705)..."
           cd scripts/mc/psipp
           ./subAna_psipp_705.sh
           ;;

    0.1.6) echo "Single D tag -- running on psipp MC sample(@707)..."
           cd scripts/mc/psipp
           ./subAna_psipp_707.sh
           ;;
    ...
    0.1.8) echo "Get samples -- applying cuts: psipp MC..."
           cd scripts/mc
           ./applyCutspsipp.sh
           ;;
