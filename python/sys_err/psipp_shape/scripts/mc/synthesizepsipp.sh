...
do
    ...
    cd /scratchfs/bes/$USER/bes/DDPIPI/v0.2/sigMC/psipp/$PARAM_0/sys_err/psipp_shape/rootfile
    ...
    if [[ $PARAM_0 == *$shortbar1* ]]; then
        dir=$(echo $PARAM_0 | sed 's/-1//g')/sys_err/psipp_shape/rootfile
        mkdir -p $dir
        mv /scratchfs/bes/$USER/bes/DDPIPI/v0.2/sigMC/psipp/$PARAM_0/sys_err/psipp_shape/rootfile/sigMC_psipp_$PARAM_0\.root ./$dir
    fi
    if [[ $PARAM_0 == *$shortbar2* ]]; then
        dir=$(echo $PARAM_0 | sed 's/-2//g')/sys_err/psipp_shape/rootfile
        mkdir -p $dir
        mv /scratchfs/bes/$USER/bes/DDPIPI/v0.2/sigMC/psipp/$PARAM_0/sys_err/psipp_shape/rootfile/sigMC_psipp_$PARAM_0\.root ./$dir
        cd $dir
