...
def make_sys():
    sample, sys_err  = [], []
    with open('./txts/sys_err_ISR.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            try:
    ...
    if not os.path.exists('./texs/'):
        os.makedirs('./texs/')
    with open('./texs/ISR_sys.tex', 'w') as f:
        f.write('\\begin{table}[htp]\n')
        f.write('\t\centering\n')
        f.write('\t\caption{Systematic uncertainty caused by initial state radiation.}\n')
