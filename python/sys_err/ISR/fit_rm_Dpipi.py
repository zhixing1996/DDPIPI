...
def fit(path, shape_path, ecms, mode, patch):
    ...
    while True:
        ...
        if mode == 'data' or mode == 'D1_2420' or mode == 'DDPIPI' or mode == 'psipp':
...
def main():
    ...
    if mode == 'data' or mode == 'none_sig' or mode == 'upper_limit':
        ...
        shape_path = '/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/mixed/sys_err/ISR/shape_' + str(ecms) + '_mixed.root'
    if mode == 'psipp' or mode == 'D1_2420':
        # path.append('/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/ISR/sigMC_' + mode + '_' + str(ecms) + '_raw_before.root')
        path.append('/besfs/groups/cal/dedx/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/ISR/sigMC_' + mode + '_' + str(ecms) + '_raw_before.root')
        shape_path = '/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/ISR/shape_' + mode + '_' + str(ecms) + '_signal.root'
    if mode == 'DDPIPI':
        # path.append('/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/ISR/sigMC_D_D_PI_PI_' + str(ecms) + '_raw_before.root')
        path.append('/besfs/groups/cal/dedx/$USER/bes/DDPIPI/v0.2/sigMC/' + mode + '/' + str(ecms) + '/sys_err/ISR/sigMC_D_D_PI_PI_' + str(ecms) + '_raw_before.root')
        shape_path = '/besfs/users/$USER/bes/DDPIPI/v0.2/ana/shape/sys_err/ISR/shape_D_D_PI_PI_' + str(ecms) + '_signal.root'
    if mode == 'MC':
        path.append('/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/mixed/sys_err/ISR/sigMC_mixed_window_' + str(ecms) + '_raw_before.root')
        shape_path = '/besfs/users/$USER/bes/DDPIPI/v0.2/sigMC/mixed/sys_err/ISR/shape_' + str(ecms) + '_mixed.root'
