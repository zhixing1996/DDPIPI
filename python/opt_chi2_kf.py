...
def cal_significance(t1, t2, t3, t4, N, step, ecms):
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(t1.GetEntries()*scale_factor(ecms, 'qq'))):
            t1.GetEntry(j)
            if t1.m_chi2_kf < (step + i*step) and fabs(t1.m_rawm_D - 1.86965) < width(ecms)/2. and fabs(t1.m_rm_Dpipi - 1.86965) < window(ecms)/2.:
                B1 = B1 + 1
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(t2.GetEntries()*scale_factor(ecms, 'DD'))):
            t2.GetEntry(j)
            if t2.m_chi2_kf < (step + i*step) and fabs(t2.m_rawm_D - 1.86965) < width(ecms)/2. and fabs(t2.m_rm_Dpipi - 1.86965) < window(ecms)/2.:
                B2 = B2 + 1
        print 'processing: ' + str(i)
        B2_list.append(B2)
    for i in xrange(N):
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(t3.GetEntries()*scale_factor(ecms, 'D1_2420'))):
            t3.GetEntry(j)
            if t3.m_chi2_kf < (step + i*step) and fabs(t3.m_rawm_D - 1.86965) < width(ecms)/2. and fabs(t3.m_rm_Dpipi - 1.86965) < window(ecms)/2.:
                S1 = S1 + 1
        print 'processing: ' + str(i)
        S1_list.append(S1)
    ...
    for i in xrange(N):
        ...
        for j in xrange(int(t4.GetEntries()*scale_factor(ecms, 'psipp'))):
            t4.GetEntry(j)
            if t4.m_chi2_kf < (step + i*step) and fabs(t4.m_rawm_D - 1.86965) < width(ecms)/2. and fabs(t4.m_rm_Dpipi - 1.86965) < window(ecms)/2.:
                S2 = S2 + 1
        print 'processing: ' + str(i)
        S2_list.append(S2)
    for i in xrange(N):
